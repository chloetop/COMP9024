...
//        preOrderPrint(clonedTree, clonedTree.root());
    ...
    /*private void cloneHelper(AVLTree<Integer, String> tree, net.datastructures.ExtendedAVLTree clonedTree, Position<Entry<Integer, String>> node, Position<Entry<Integer, String>> clonedNode){
        Position<Entry<Integer, String>> tempNode = null;
        if(node.element() != null){
            if(!tree.isRoot(node)){
                tempNode =
                        clonedTree.createNode(node.element(),
                                (BTPosition<Entry<Integer,String>>)(clonedNode),
                                (BTPosition<Entry<Integer,String>>)(left(node)),
                                (BTPosition<Entry<Integer,String>>)(right(node)));
                clonedTree.setHeight(node);
            }else{
                tempNode =
                        clonedTree.createNode(node.element(), null,
                                (BTPosition<Entry<Integer,String>>)(left(node)),
                                (BTPosition<Entry<Integer,String>>)(right(node)));
                clonedTree.setHeight(node);
                clonedTree.root = (BTPosition<Entry<Integer,String>>)tempNode;
            }
        }
        if(tree.isInternal(node) && tree.hasLeft(node)){
            cloneHelper(tree,clonedTree, tree.left(node), tempNode);
        }
        if(tree.isInternal(node) && tree.hasRight(node)){
            cloneHelper(tree,clonedTree, tree.right(node), tempNode);
        }
    }*/

    /*private Position<Entry<Integer, String>> cloneHelper(Position<Entry<Integer, String>> node, Position<Entry<Integer, String>> clonedNode){
        Position<Entry<Integer, String>> leftNewNode = null;
        Position<Entry<Integer, String>> rightNewNode = null;
        if(isInternal(node) && hasLeft(node) && node.element() != null){
            leftNewNode = createNode(node.element(), null, null, null);
            cloneHelper(left(node), leftNewNode);
        }
        if(isInternal(node) && hasRight(node) && node.element() != null){
            rightNewNode = createNode(node.element(), null, null, null);
            cloneHelper(right(node), rightNewNode);
        }
        if(leftNewNode != null && leftNewNode.element() != null){
            this.insertLeft(clonedNode, leftNewNode.element());
        }
        if(rightNewNode != null && rightNewNode.element() != null)
            this.insertRight(clonedNode, rightNewNode.element());

        return node;
    }*/

    /*public static AVLTree<Integer, String> clone(AVLTree<Integer, String> tree){
        net.datastructures.ExtendedAVLTree extendedAVLTree = new net.datastructures.ExtendedAVLTree();
        net.datastructures.ExtendedAVLTree clonedTree =  new net.datastructures.ExtendedAVLTree();
        extendedAVLTree.cloneHelper(tree.root(), clonedTree.root());
        //extendedAVLTree.cloneHelper(tree, clonedTree, tree.root(), clonedTree.root());
        System.out.println("Original Tree");
        extendedAVLTree.preOrderPrint(tree, tree.root());
        System.out.println(" ");
        System.out.println("Cloned Tree");
        extendedAVLTree.preOrderPrint(clonedTree, clonedTree.root());
        tree.insert(50, "TestNode");
        tree.insert(65, "TestNode One More");
        System.out.println(" ");
        System.out.println("Original Tree");
        extendedAVLTree.preOrderPrint(tree, tree.root());
        System.out.println(" ");
...
/**
 *
 * This class contains the node setting such as height and width of the node.
 * THis is being used to draw nodes.
 *
 */
